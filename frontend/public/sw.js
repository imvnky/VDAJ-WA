...
self.addEventListener('fetch', (event) => {
  ...
  // Skip cross-origin, WebSockets, and non-GET requests
  ...
  // 2. HTML navigation & index.html requests: Network-First (never serve stale index.html)
  ...
  // 3. Static assets (/assets/*.js, /assets/*.css): Cache-First
  event.respondWith(
    caches.match(request).then((cached) => {
      if (cached) return cached;
      return fetch(request).then((res) => {
        // Prevent returning HTML (SPA fallback) for missing JS/CSS bundles
